...
export default function LoginPage() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Pass pending org slug if user came from /{orgSlug} landing page
    ...
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden" style={{ background: "linear-gradient(135deg, #FFF5EC 0%, #FFFFFF 100%)" }}>
      ...
      <div className="relative z-10 w-full max-w-md px-6">
        <div className="text-center mb-10">
          ...
          <p className="mt-2 text-lg" style={{ color: "#666666" }}>Intelligent task tracking with WhatsApp</p>
        </div>

        <div className="bg-white/80 backdrop-blur-xl rounded-[2rem] shadow-xl p-8 border border-white/50">
          <h2 className="text-xl font-bold mb-1" style={{ color: "#1A1A1A" }}>Welcome back</h2>
          <p className="text-sm mb-8" style={{ color: "#666666" }}>Sign in to manage your tasks and teams</p>
        ...
        <div className="mt-8 grid grid-cols-3 gap-4 text-center">
          {[
            ...
            <div key={f.label}>
              ...
              <div className="text-xs" style={{ color: "#666" }}>{f.label}</div>
            </div>
          ))}
        </div>
      </div>
