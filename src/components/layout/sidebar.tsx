...
export default function Sidebar({ orgs, user }: SidebarProps) {
  ...
  const [waStatus, setWaStatus] = useState<"CONNECTED" | "QR_PENDING" | "DISCONNECTED">("DISCONNECTED");
  ...
  useEffect(() => {
    ...
    const loadStatus = async () => {
      try {
        const res = await fetch(`/api/whatsapp/status?orgId=${orgId}`, { cache: "no-store" });
        if (!res.ok) return;
  ...
  }, [mounted, currentOrg?.id, orgs]);

  const waIndicator = waStatus === "CONNECTED"
    ? {
        bg: "bg-[#25D366]/10",
    ...
    <aside className="hidden md:flex w-[248px] shrink-0 flex-col h-screen border-r border-primary-500/10 shadow-[inset_-1px_0_0_rgba(255,255,255,0.65)] bg-gradient-to-b from-[#FFFDF9] to-[#FFF5EC]">
      ...
      <div className="p-[0_10px_8px]">
        <Link
          ...
          className={`flex items-center gap-2.5 p-[10px_12px] rounded-xl border transition-colors ${waIndicator.bg} ${waIndicator.border} ${waIndicator.hover}`}
        >
          <svg width="15" height="15" fill={waIndicator.iconFill} viewBox="0 0 24 24">
          ...
          <span className={`text-[12px] flex-1 ${waIndicator.text}`}>{waIndicator.label}</span>
          <span className={`w-[7px] h-[7px] rounded-full inline-block shrink-0 ${waIndicator.dot}`} />
        </Link>
      </div>
